...
//! - The worker holds an `Arc<Mutex<Session>>` clone — the
...
impl AutosaveHandle {
    ...
    pub fn start(
        ...
        let thread = std::thread::Builder::new()
            ...
            .spawn(move || autosave_loop(session, dirty, config, stop_clone))
...
fn autosave_loop(
    ...
    loop {
        ...
        match write_one_tick(&session, &dirty, &config.dir, &mut last_written_seq) {
...
fn write_one_tick(
    ...
    // Clone the persisted project + read the recovery origin under the lock,
    // then write off-lock. The origin rides the recovery file's envelope.
    let (snapshot, origin) = {
        let s = session.lock().expect("session poisoned");
        (s.project.clone(), s.runtime.autosave_origin().map(Path::to_path_buf))
    };
    let path = autosave_path_for(dir, &snapshot);
    write_autosave(&snapshot, &path, origin.as_deref())?;
...
mod tests {
    ...
    fn write_one_tick_skips_when_clean() {
        ...
        let dir = tempdir();
        let project = Arc::new(Mutex::new(Session::new(Project::default())));
    ...
    fn write_one_tick_writes_when_dirty() {
        ...
        let expected = autosave_path_for(dir.path(), &project.lock().unwrap().project);
    ...
    fn scan_recoveries_lists_written_autosaves() {
        ...
        let p1 = Arc::new(Mutex::new(Session::new(Project::default())));
        let d1 = DirtyTracker::new();
        note_edit(&d1);
        write_one_tick(&p1, &d1, dir.path(), &mut None).expect("ok");
        let p2 = Arc::new(Mutex::new(Session::new(Project::default())));
        ...
        let want1 = p1.lock().unwrap().project.uuid.to_string();
        let want2 = p2.lock().unwrap().project.uuid.to_string();
    ...
    fn drop_autosave_removes_file() {
        ...
        let uuid = project.lock().unwrap().project.uuid.to_string();
    ...
    fn autosave_stamps_recovery_origin_from_source_path() {
        let dir = tempdir();
        let project = Arc::new(Mutex::new(Session::new(Project::default())));
        project.lock().unwrap().runtime.source_path = Some(PathBuf::from("/home/u/foo.n3o"));
        let dirty = DirtyTracker::new();
        note_edit(&dirty);
        write_one_tick(&project, &dirty, dir.path(), &mut None).expect("ok");
        ...
        let path = autosave_path_for(dir.path(), &project.lock().unwrap().project);
        let (_, recovery_origin) = super::super::format::read_project(&path).expect("read");
        assert_eq!(
            recovery_origin,
    ...
    fn autosave_of_untitled_project_leaves_recovery_origin_none() {
        ...
        let path = autosave_path_for(dir.path(), &project.lock().unwrap().project);
        let (_, recovery_origin) = super::super::format::read_project(&path).expect("read");
        assert!(recovery_origin.is_none());
    }
    ...
    fn autosave_preserves_recovery_origin_for_untitled_recovered_project() {
        ...
        let project = Arc::new(Mutex::new(Session::new(Project::default())));
        {
            ...
            p.runtime.source_path = None;
            p.runtime.recovery_origin = Some(PathBuf::from("/home/u/orig.n3o"));
        }
        let dirty = DirtyTracker::new();
        note_edit(&dirty);
        write_one_tick(&project, &dirty, dir.path(), &mut None).expect("ok");
