//! Bambu AMS slot-mapping for a plate.
//!
//! Translates a plate's `material_to_slot` map + the bound
//! PrinterInstance topology into the AMS routing shapes Bambu
//! firmware consumes: the `.gcode.3mf` `ams_bindings` list and the
//! MQTT `project_file` `(use_ams, ams_mapping, ams_mapping2)`
//! triple. Both are indexed by model material / filament position,
//! not by PrinterInstance slot position.
