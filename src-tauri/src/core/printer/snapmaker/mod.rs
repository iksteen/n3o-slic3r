...
//! holds the actual Moonraker HTTP+WS client. FR-SU-1
//! through FR-SU-9 are split — the cascade fields (FR-SU-6/7/8/9)
//! live here; the network surface (FR-SU-1/2/3/4/5) lives in the
//! driver module.
//!
//! Architecture clarification (see PRD AD-7): the U1 exposes vanilla
//! Moonraker on plain HTTP+WS port 80, not a Snapmaker-proprietary
//! wrapper. The Snapmaker-specific pair / mTLS / MQTT control plane
//! (used for the webcam) is implemented under `core/driver/snapmaker/`
//! and drives the U1 camera.
