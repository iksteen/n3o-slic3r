...
| `cube-plain.3mf`, `cube-override.3mf` | `cube-override-pair.py` | One 20mm cube, identical geometry, exported twice — plain, and with an object-scoped `<metadata key="layer_height" value="0.25">` override (how a foreign Orca export records an object-level setting). The import-override end-to-end test imports both and asserts the override populates `scene.object_overrides` and visibly changes the slice (fewer layers). |
| `prusa-cube.3mf` | `prusa-cube.py` | A single 20mm cube in **PrusaSlicer / Slic3r PE** flavor: standard-core geometry with the `slic3rpe` namespace, a `Metadata/Slic3r_PE_model.config` (Prusa's `<object>`/`<volume>` per-object metadata), and a `Metadata/Slic3r_PE.config` (INI print config, shell-only: `top/bottom_solid_layers = 0`, `perimeters = 6`, `fill_density = 15%`, plus printer/temp noise). No BBS `model_settings.config`, no `project_settings.config`. Exercises the Prusa geometry import, the object-name lift (`type="object"` name, not the sibling `<volume>` name), and the geometry-intent override lift (shells/walls/infill/patterns/raft/brim/seam → OrcaSlicer per-object overrides; printer/filament keys AND print-global skirt/spiral keys dropped). |
