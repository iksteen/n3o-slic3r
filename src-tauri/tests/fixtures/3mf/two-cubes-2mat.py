#!/usr/bin/env python3
"""Two 20mm cubes side by side, each with its own extruder hint
(cube A → material 1, cube B → material 2). Hand-authored 3mf
matching the shape n3o-slic3r's own writer produces:
...
Output: `two-cubes-2mat.3mf` next to this script. Regenerate via
  python3 src-tauri/tests/fixtures/3mf/two-cubes-2mat.py
"""
import zipfile
...
def obj_xml(oid, v, t):
    ...
    return f'  <object id="{oid}" type="model">\n   <mesh>\n    <vertices>{vx}</vertices>\n    <triangles>{tx}</triangles>\n   </mesh>\n  </object>\n'

v, t = cube(20.0)
model = ['<?xml version="1.0" encoding="UTF-8"?>\n<model unit="millimeter" xml:lang="en-US" xmlns="http://schemas.microsoft.com/3dmanufacturing/core/2015/02">\n']
model.append(' <metadata name="Application">n3o-2cube-fixture-builder</metadata>\n')
model.append(' <resources>\n')
model.append(obj_xml(1, v, t)); model.append(obj_xml(2, v, t))
model.append(' </resources>\n <build>\n')
model.append('  <item objectid="1" transform="1 0 0 0 1 0 0 0 1 90 60 10" printable="1"/>\n')
model.append('  <item objectid="2" transform="1 0 0 0 1 0 0 0 1 90 120 10" printable="1"/>\n')
model.append(' </build>\n</model>\n')

ms = ['<?xml version="1.0" encoding="UTF-8"?>\n<config>\n']
for oid, name, ext in [(1,"Cube A (T0)",1),(2,"Cube B (T1)",2)]:
    ms.append(f'  <object id="{oid}">\n')
    ms.append(f'    <metadata key="name" value="{name}"/>\n')
    ms.append(f'    <metadata key="extruder" value="{ext}"/>\n')
    ms.append(f'    <part id="1" subtype="normal_part">\n')
    ms.append(f'      <metadata key="name" value="{name}"/>\n')
    ms.append(f'      <metadata key="extruder" value="{ext}"/>\n')
    ms.append('    </part>\n  </object>\n')
...
with zipfile.ZipFile(OUT, "w", compression=zipfile.ZIP_DEFLATED) as z:
    ...
    z.writestr("Metadata/n3o_project_settings.config", '<?xml version="1.0" encoding="UTF-8"?>\n<n3o_project version="1" writer="n3o-2cube-fixture-builder">\n</n3o_project>\n')
print(f"wrote {OUT}")
