...
`docs/profiles.md` "Syntax — three equivalent forms": top-level
...
- All three syntactic forms desugar to the same in-memory `Rule`
  ...
  documented in `docs/profiles.md` — top-level keys at source
...
alternate formats (TOML only per `docs/profiles.md`).
