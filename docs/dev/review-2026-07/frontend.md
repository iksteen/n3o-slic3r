...
> **STATUS — fully executed (2026-07-18).** All 14 findings are done. The last open one, FE-X3's `getUserProcess`/`printerInstanceSetBed` dead wrappers, was removed in `a9cb62a`; the rest were addressed earlier. Every checkbox below was re-verified against current code.
...
- [x] **FE-D1 — `DevicesView` header describes a removed implementation**
...
- [x] **FE-X1 — Dead components `BambuAmsStrip` + `U1ToolheadStrip` (~220 LOC)** *(MEDIUM; ponytail)*
...
- [x] **FE-X2 — Dead module `buildContextJson.ts` + orphaned cascade types (client-builds-context vestige)** *(MEDIUM; ponytail)*
...
- [x] **FE-X3 — Dead invoke wrappers** *(ponytail)*
...
- [x] **FE-X4 — Dead re-export `defaultWindow` in `LayerSlider`** *(ponytail)*
...
- [x] **FE-X5 — `Modal` speculative props no caller exercises** *(ponytail)*
...
- [x] **FE-X6 — Duplicated Vec3 primitives between `WgpuViewport` and `useSplitSession`** *(ponytail)*
...
- [x] **FE-C1 — Cascade resolve goes stale after a build-plate or nozzle-diameter change** *(MEDIUM)*
...
- [x] **FE-C2 — `WgpuViewport` fires a full GPU frame + framebuffer IPC blit on every App re-render** *(MEDIUM)*
...
- [x] **FE-C3 — Dirty flag: a late-resolving initial fetch can overwrite a newer event** *(LOW; arch)*
...
- [x] **FE-C4 — `listenPlateEdits` is needlessly `async`; the await opens a mount/unmount cleanup race** *(LOW; ponytail/arch)*
...
- [x] **FE-C5 — `useSlicePreviewBridge` async listener can leak if unmounted before `listenPlateEdits` resolves** *(LOW; arch)*
