...
> **STATUS — fully executed (2026-07-18).** All 19 findings are done and committed (`5612d0d` Phase 1+2, `7c2cff2` sibling FFI Phase 3, the model refactors, and the register-purity series). Every checkbox below was re-verified against current code before ticking.
...
- [x] **T-B1 — `clone_objects` / `move_objects_to_plate` silently drop cut connectors** *(HIGH)*
...
- [x] **T-D1 — `JobRegistry::remove` doc contradicts the now-live C-12 prune**
...
- [x] **T-X1 — `plate_cascade_trace` command + trace machinery is dead** *(ponytail)*
...
- [x] **T-X2 — `user_process_get` command is dead** *(ponytail)*
...
- [x] **T-X3 — `process:user_changed` event has no listener** *(ponytail)*
...
- [x] **T-X4 — `PlateMetadata` / `composition_order` is speculative dead state** *(ponytail)*
...
- [x] **T-X5 — Dead cascade file-loaders `load_cascade` / `load_override_file`** *(ponytail)*
...
- [x] **T-X6 — `SemanticComment::ExtruderTemp`/`BedTemp` + `LayerSource::Heuristic` dead variants** *(ponytail)*
...
- [x] **T-X7 — `default_printer_identity` + `DEFAULT_IDENTITY` OnceLock dead in the shipped binary** *(ponytail)*
...
- [x] **T-X8 — `PluginHost::set_enabled` + `find_mut` dead outside tests** *(MEDIUM; ponytail)*
...
- [x] **T-X9 — `PayloadKind::Gcode3mf` variant never constructed** *(ponytail)*
...
- [x] **T-X10 — Camera worker reimplements exponential backoff** *(ponytail)*
...
- [x] **T-X11 — `engine_default_serialized` rebuilds the whole ~600-entry option table to read one key's default** *(MEDIUM; ponytail)* — pairs with part-1 **FFI-CACHE**.
...
- [x] **T-C1 — `instance_storage::persist` writes instance TOML non-atomically → a torn write drops the whole printer (incl. saved credentials)** *(MEDIUM)*
...
- [x] **T-C2 — Renderer `GpuMesh` cache grows unbounded within a session (GPU buffer leak)** *(MEDIUM)*
...
- [x] **T-C3 — `read_rgba` panics on device loss / map failure and poisons `ViewportState`, bricking the viewport for the session** *(LOW; arch)*
...
- [x] **T-C4 — `printer_instance_delete_with_reassign` commits plate rebinds but drops their events when `delete_instance` errors** *(LOW; arch)*
...
- [x] **T-C5 — Bambu status worker clobbers the event loop's `Reconnecting` state with a stale `Connected`** *(LOW; arch)*
...
- [x] **T-C6 — `SendCancelRegistry` assumes the driver lock serializes sends, but `send()` takes a shared read lock** *(LOW; arch)*
