...
- [x] **C-1 — Wire the two-phase override-tier path in as the live resolver** *(DP-1; the central cascade task)*
  ...
  **Done.** `compose_cascade` no longer bakes overrides (Layer 6 removed; the `plate_overrides` param is gone); machine overrides (5b) stay baked as the instance's own config. Both the slice worker and the panel `resolve_instance_cascade` now compose the authored cascade, build `OverrideTiers` (user = `Project.user_overrides`, project = `Plate.project_overrides`, object deferred), and run `cascade::resolve_with_overrides` → `cascade::to_resolved` (new flatten) → unchanged hook/safety-gate/`adapt`. **Parity guard** `composer::tests::project_tier_matches_baked_important_rule` proves tier ≡ baked-`!important` (incl. an override-only key) before the swap. The user→engine gap is closed and proven via G-code (`slice_orchestrator::user_tier_override_reaches_the_engine`). Trace: `project::commands::plate_cascade_trace(plate_id, key)` resolves with full tiers and returns `cascade::Trace` (winning tier + cascade fallback + losing rules); `trace_attributes_a_project_override_to_the_project_tier` guards the wiring. `adapt_with_overrides` was redundant after the swap (the plugin hook mutates the flattened map before `adapt`) and is **deleted**. Deferred: the trace *popover UI* (the command + `cascade::Trace` are the consumable seam; no popover ships here) and per-object slice-time overrides (object tier stays empty, the pre-existing deferral).
...
## Phase 4 — Mechanical refactors & relocations (last)  ✅ DONE (14/15; R-15 deferred as documented debt — cargo test --workspace --features test-fixtures: 945)
...
- [x] **R-2 — Move settings-UI option surfacing out of `core/cascade`** *(landed in `printer`, not `schema` — see note)*
  ...
  **Done — relocated to `core/printer/options.rs`, not `schema`.** The plan's `schema` target conflicts with the already-done R-4: option-surfacing depends on `capability_for_key`/`CapabilityPredicate` + `PrinterProfile` (both now in `printer`) plus `slic3r_ffi` directly — putting it in `schema` would re-add `schema→printer` and undo R-4's FFI-leaf goal. It uses **no** `core::schema` and no resolver, so `printer` is the natural home (its deps already live there). Result: `cascade/mod.rs` is now just module decls + resolver re-exports and imports nothing from `printer` (verified). The four `slicer_*` commands re-register at `core::printer::options::*`; frontend is unaffected (commands invoked by name). The OptionSchema cache-folding (the perf sub-goal) is **not** done — the surface still calls `slic3r_ffi::option_defs()` per command; left as a separate optimization since the architectural move (out of cascade) is the win.
...
- [x] **R-10 — Consolidate duplicated helpers** *(2 of 3; cstyle deferred with rationale)*
  ...
  **Done — except cstyle (deliberately not merged).** `strip_prefix_ci` (the *actual* shared comment primitive — `summary.rs` reads parsed `HeaderMetadata`, it isn't a recognizer) → one `pub(crate)` fn in `gcode/mod.rs`, both gcode scanners point at it. `merge_status` → pure `pub(super) merge_status_into` in `moonraker.rs`; the method delegates and both test modules call it (no more inline copies). **cstyle is NOT a mechanical dedup:** the two ports have *deliberately different* semantics — the cascade one skips whitespace around the `;` separator (to mirror libslic3r's parse of FFI-emitted defaults) and emits a trailing-`;`→empty entry; the composer one does neither (it only ever splits values it serialized itself, which never carry stray whitespace or a trailing `;`). Unifying is behavior reconciliation with libslic3r-string-parse regression risk, not a ~30-line win — left as documented intentional duplication.

- [x] **R-11 — Shared wgpu device + extract the pure gizmo solver**
