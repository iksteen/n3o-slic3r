...
- coEnums default surfacing — done in commit `1bb3503`. All 9 affected option keys now expose a reverse-looked-up default via `def.enum_keys_map`. See `docs/dev/libslic3r-workarounds.md` §5.
...
- Pointer to the profiles strategy document (docs/dev/profiles.md or similar) as the source of truth for the rule cascade design, TOML schema, resolution semantics, and translation adapter. The PRD §6.1 names the requirements; the strategy doc owns the design.
