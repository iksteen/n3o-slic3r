...
Status: ✅ shipped — slice progress callback + log sink redirect both done. **Progress**: `slic3r_set_slice_progress_cb` (mutex-guarded global, installed via `PrintBase::set_status_callback` for each `slic3r_slice` call; emits silently when no callback registered). **Log sink**: `slic3r_set_log_sink` with a custom `synchronous_sink<CallbackLogBackend>` installed into the boost::log core at `slic3r_init` time; sink stays for the process lifetime and no-ops when no callback registered. Severity ladder mirrors `boost::log::trivial::severity_level` (0=trace…5=fatal). Throwing callbacks are swallowed inside the backend to keep libslic3r's logging path crash-safe. Rust bindings mirror the progress pattern (`Mutex<Option<Box<dyn FnMut + Send>>>` + `extern "C"` trampoline). 4 integration tests: progress callback fires across 100+ ticks reaching "Generating G-code", clearable; log sink receives non-empty records during slice, clearable. Tests serialize through process-local mutexes since the FFI registries are global.
...
- Docs: extend `docs/dev/libslic3r-workarounds.md` with a new section
