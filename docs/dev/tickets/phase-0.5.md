...
Source: `docs/dev/Execution_Plan.md` §2.5. Stated exit criteria:
...
| Spike | Status | Ticket |
|-------|--------|--------|
| Cascade adapter end-to-end | ✅ done | [PR-0.5-1](phase-0.5/PR-0.5-1%20Cascade%20Adapter%20end-to-end.md) (finding: `docs/dev/spikes/spike-1-cascade-adapter.md`) |
| Mixed-nozzle-size slice (Prusa XL) | ✅ done | [PR-0.5-2](phase-0.5/PR-0.5-2%20Mixed-nozzle-size%20slice.md) (finding: `docs/dev/spikes/spike-2-mixed-nozzle.md`; toolchange criterion confirmed by PR-0.5-3) |
| Bambu A1 mini AMS slice | ⚠️ done with Phase-5 prerequisite | [PR-0.5-3](phase-0.5/PR-0.5-3%20Bambu%20A1%20mini%20AMS%20slice.md) (finding: `docs/dev/spikes/spike-3-bambu-ams.md`; BBS comparison surfaces ~10× more tool-changes than BBS — Phase 5 must solve before hardware validation) |
| coEnums known limitation impact | ✅ done | — (surfaced via `1bb3503`; documented in `docs/dev/libslic3r-workarounds.md` §5) |
| platecycler portability | ✅ done | [PR-0.5-5](phase-0.5/PR-0.5-5%20platecycler%20portability.md) (finding: `docs/dev/spikes/spike-5-platecycler.md`) |

Findings docs land at `docs/dev/spikes/spike-<n>-<slug>.md`. The Spike 4
finding is inline in `docs/dev/libslic3r-workarounds.md`; the others
get dedicated files.
...
and any updates to `docs/dev/profiles.md`,
`docs/dev/libslic3r-workarounds.md`, or the FFI shim that the spikes
prompted.
...
- Four new finding docs at `docs/dev/spikes/spike-{1,2,3,5}-*.md`
  (Spike 4's finding is in `libslic3r-workarounds.md` §5).
- Any failed spike has a plan-revision PR open against
  `docs/dev/PRD.md` or `docs/dev/Execution_Plan.md`.
- `docs/dev/libslic3r-workarounds.md` reflects any newly discovered
...
- The Phase 0.5 milestone (`M0.5 — Engine assumptions validated` in
  `docs/dev/Execution_Plan.md`) ticks over.
