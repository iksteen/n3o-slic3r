...
PlateCycler (see `docs/dev/phase-8-platecycler-smoke.md`).
...
- Hardware smoke method in `docs/dev/phase-8-platecycler-smoke.md` (result
...
- A bundled plugin `resources/plugins/platecycler/` (`plugin.toml` +
  ...
  - Declares a `swap_gcode` string setting (default = the
    ...
    `docs/dev/spikes/spike-5-platecycler.md`), so a user can tune the macro
...
- **Real-hardware smoke** (the exit-criteria proof): slice a print with
  ...
  in `docs/dev/phase-8-platecycler-smoke.md` (assumption, method, result —
...
`docs/dev/spikes/spike-5-platecycler.md` supplies the macro.
