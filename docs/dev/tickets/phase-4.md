...
Source: `docs/dev/Execution_Plan.md` §6. Stated goal:
...
shell that hosts it) lives in `docs/dev/design/`. Open
`docs/dev/design/index.html` (or the self-contained
...
`docs/dev/design/app.jsx` carefully before lifting anything to a
...
Canonical defaults (from `TWEAK_DEFAULTS` in `docs/dev/design/app.jsx`):
...
- **Cascade-layer vocabulary** (`docs/dev/design/data.jsx`): the seven
  ...
  reinvent.
- **Settings-row anatomy** (`docs/dev/design/SettingsPanel.jsx`,
  ...
  etc.) from `docs/dev/design/styles.css` so designers can iterate
...
- **Theming and palette** (`docs/dev/design/styles.css`): `oklch()`
