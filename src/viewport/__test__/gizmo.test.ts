...
import type { MeshHeader, SceneObject } from "../types";
...
describe("gizmo.pickAttachTarget", () => {
  it("returns null when nothing's selected", async () => {
    ...
    await mirror.applyEvent({ kind: "ObjectAdded", data: objAt(101, 1, 0) });
  ...
  it("finds the Three.js mesh whose userData matches the first selected id", async () => {
    ...
    await mirror.applyEvent({ kind: "MeshLoaded", data: unitCubeHeader(1) });
    await mirror.applyEvent({ kind: "ObjectAdded", data: objAt(101, 1, 0) });
    await mirror.applyEvent({ kind: "ObjectAdded", data: objAt(102, 1, 30) });
  ...
  it("multi-select attaches to the FIRST selected id (PR-2-10 known limitation)", async () => {
    const mirror = new SceneMirror(async () => unitCubeBuffers());
    await mirror.applyEvent({ kind: "MeshLoaded", data: unitCubeHeader(1) });
    await mirror.applyEvent({ kind: "ObjectAdded", data: objAt(201, 1, 0) });
    await mirror.applyEvent({ kind: "ObjectAdded", data: objAt(202, 1, 30) });
