...
describe("SceneMirror", () => {
  ...
  it("renders a bed overlay on BedChanged", async () => {
    ...
    // Grid + outline + X/Y/Z axes + 1 zone wireframe = 6 children.
    expect(mirror.bedChildCount()).toBe(6);
...
describe("SceneMirror spool-color paint", () => {
  ...
  it("Snappy: object routed to a higher-extruder slot paints with that extruder's color", async () => {
    ...
    const snappy: PrinterInstance = {
      ...
      connection: null,
      extruders: [
        { installed_nozzle: { diameter: "0.4", material: "stainless" }, slots: [{ feed: "direct", filament_identity: "generic-pla", color: "#dc2626", tag_uid: null }] },
        { installed_nozzle: { diameter: "0.4", material: "stainless" }, slots: [{ feed: "direct", filament_identity: "generic-pla", color: "#eab308", tag_uid: null }] },
        { installed_nozzle: { diameter: "0.4", material: "stainless" }, slots: [{ feed: "direct", filament_identity: "generic-pla", color: "#111827", tag_uid: null }] },
        { installed_nozzle: { diameter: "0.4", material: "stainless" }, slots: [{ feed: "direct", filament_identity: "generic-pla", color: "#f8fafc", tag_uid: null }] },
      ],
